...
# imagenary number support
# failure safe for double decimal and double negative
# failure safe? whats that?
# better terminology
# simplier and compact code
# cutting unnesscessery lines
# better Error naming
...
# failure safe
# loop
# clear function at the end only
# decimal support
# decimal negative support
# percentage support
# negative numbers support
# okayy gng, I no longer know what im doing
...
while True:
    ...
    while True:
        ...
        if result.isdigit():
            ...
            elif not any(digit.isalpha() for digit in result):
                number_1 = float(result)
                print(f"first number: {number_1}")
                break
            ...
            # a failure safe to prevent failures if result has [e] in it [1.6e *10]
                ...
                print("Error: Invalid result dot value")
                result = "loop break"

        elif result.find("-") != -1:

            if any(char.isalpha() for char in result) or result.split("-")[0].isdigit():
            ...
            elif result.split("-")[1].isdigit():
                number_1 = int(result)
                print(f"first number: {number_1}")
                break

            else:
                print("Error: Invalid decimal.")
        ...
        # to input the first number
            ...
            if number_1.find(".") != -1:
                ...
                elif not any(digit.isalpha() for digit in number_1):
                    number_1 = float(number_1)
                    break
            ...
            elif number_1.find("-") != -1:

                if any(char.isalpha() for char in number_1) or number_1.split("-")[0].isdigit():
                    print("Error: Invalid negative number")

                elif number_1.split("-")[1].isdigit():
                    number_1 = int(number_1)
                    break
    ...
    while True:
        ...
        if number_2.find(".") != -1:
            ...
            elif not any(digit.isalpha() for digit in number_2):
                number_2 = float(number_2)
        ...
        elif number_2.find("-") != -1:

            if any(char.isalpha() for char in number_2) or number_2.split("-")[0].isdigit():
                print("Error: Invalid negative number")

            elif number_2.split("-")[1].isdigit():
                number_2 = int(number_2)
                if number_2 == 0 and operation == "/" or operation == "//":
                    print("Error: Division by zero")
                else:
